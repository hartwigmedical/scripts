...
library(readr)
library(stringr)
libgridssfile = paste0(argp$scriptdir, "libgridss.R")
if (file.exists(libgridssfile)) {
  ...
  msg = paste("Could not find libgridss.R in", argp$scriptdir, " - please specify a --scriptdir path to a directory containing the required scripts")
  write(msg, stderr())
  print(argp)
  stop(msg)
}
