...
{
  evaluate_signature_fit(runType, runId, signatures, contribution, matrixData, summaryCounts, sampleCancerTypes, bucketNames, sigNamesNamed, 
                         plotByCancerType, viewResults, bgSigCount, printAllPlots)
}
                            ...
                            sigInfo = data.frame(), sigAllocs = data.frame())
{
  ...
  if(printAllPlots)
  {
    # 3. Plot of Bucket Percents for each Sample
  ...
  if(printAllPlots | !plotByCancerType)
  {
