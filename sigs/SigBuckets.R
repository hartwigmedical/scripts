sample_counts_to_matrix<-function(sampleCounts)
{
  matrixData = sampleCounts %>% spread(SampleId,Count)
  matrixData[is.na(matrixData)] <- 0
  matrixData = within(matrixData, rm(Bucket))
  return (matrixData)
}

matrix_to_sample_counts<-function(matrixData, bucketNames)
{
  bucketCounts = cbind(bucketNames,matrixData)
  gatherIndex = ncol(bucketCounts)
  sampleCounts = gather(bucketCounts, "SampleId", "Count", 2:gatherIndex) %>% select(SampleId,Bucket,Count)
  return (sampleCounts)
}
