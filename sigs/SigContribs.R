...
calc_sig_alloc_sample_residuals<-function(sigSampleBucketCounts, matrixData, bucketNamesIndexed)
{
  ...
  sampleBucketFitVsActuals = (sampleBucketFitVsActuals %>% mutate(ResidualExcess=ifelse(SigCount > ActualCount, SigCount - ActualCount, 0),
                                                                  ResidualUnalloc = ifelse(ActualCount > SigCount, ActualCount - SigCount, 0),
                                                                  ResidualDiff = abs(ActualCount-SigCount)))
...
calc_contrib_sample_residuals<-function(contributions, signatures, matrixData, bucketNames)
{
  ...
  sampleBucketFitVsActuals = (sampleBucketFitVsActuals %>% mutate(ResidualExcess=ifelse(FitCount > ActualCount, FitCount - ActualCount, 0),
                                                                  ResidualUnalloc = ifelse(ActualCount > FitCount, ActualCount - FitCount, 0),
                                                                  ResidualDiff = abs(ActualCount - FitCount)))
