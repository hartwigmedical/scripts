...
for FILE in $CRT_DIR/FA_hmfuploaduser.crt $CRT_DIR/FA_hmfuploaduser.pem; do
  CheckFileExists $FILE
done

if [ -f /tmp/cookie.tmp ]; then
  /bin/rm -f /tmp/cookie.tmp
fi

# We need an empty file to hold cookie session
/bin/touch /tmp/cookie.tmp

# Authenticate to connect.schubergphilis.com to create cookie
/usr/bin/curl -s -L -k --cert-type pem --cert $CRT_DIR/FA_hmfuploaduser.crt --key $CRT_DIR/FA_hmfuploaduser.pem -b /tmp/cookie.tmp -c /tmp/cookie.tmp https://connect.schubergphilis.com/hmfdashboard

# Cookie now will allow access to api.schubergphilis.com for a short time
/usr/bin/curl -s -v -L -k \
  --cert-type pem \
  --cert $CRT_DIR/FA_hmfuploaduser.crt \
  --key $CRT_DIR/FA_hmfuploaduser.pem \
  -b /tmp/cookie.tmp \
  -c /tmp/cookie.tmp \
  https://api.schubergphilis.com/hmf/v1/sets \
  -XPOST \
