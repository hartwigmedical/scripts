...
where clinical.hasSystemicPretreatment is null and ecrf.item='FLD.PRETHERAPY.SYSTEMIC' and locked ='false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.hasRadiotherapyPreTreatment is null and ecrf.item ='FLD.PRETHERAPY.RADIOTHER' and locked ='false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.preTreatments is null and clinical.hasSystemicPretreatment = 'yes' and ecrf.item='FLD.PRETHERAPY.SYSTEMICREG' and locked ='false';
...
and details='ecrf biopsies: 0; sequenced: 1' and formLocked = 'false';

# 1 ingevulde ecrf formulier en 2 biopten gesequenced
select * from clinicalFindings where ecrfItem = 'FRM.BIOPS' and message ='less ecrf biopsies than biopsies sequenced.'
and details='ecrf biopsies: 1; sequenced: 2' and formLocked = 'false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.treatmentGiven is null and ecrf.item='FLD.TRTAFTER.SYSTEMICST' and locked ='false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.treatmentStartDate is null and ecrf.item='FLD.TRTAFTER.SYSSTDT' and locked='false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.treatment is null and ecrf.item='FLD.TRTAFTER.SYSREGPOST' and locked='false';
...
inner join patient on sample.patientId=patient.id
inner join ecrf on ecrf.patientId=patient.cpctId
where clinical.radiotherapyGiven is null and ecrf.item='FLD.TRTAFTER.RADIOTHERST' and locked ='false';
