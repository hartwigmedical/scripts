...
WHERE ((pathogenic = 'UNANNOTATED' AND codingEffect IN ('NONSENSE_OR_FRAMESHIFT','SPLICE') AND reported = 0)						                    #also check for filter here
...
OR (gene IN ('NTHL1','MUTYH') AND reported = 1 AND refStatus = 'HET')                                                                                   #these genes should only be marked with patient notification in case of refStatus = homozygous
