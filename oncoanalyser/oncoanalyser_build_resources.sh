...
print_usage() {
    ...
    echo ""
    echo "Usage:"
    echo "  bash oncoanalyser_build_resources <dest_dir> <ref_genome_version> <resources_version> <mode>"
    echo ""
    echo "Arguments:"
...
echo "Resources version: ${RESOURCES_VERSION}"
echo "## --------------------------------"
...
make_tar (){
	...
	echo "Creating tar with basename: $TARGET_BASENAME_NO_EXT"
	tar --use-compress-program="pigz -9" -cvhf $TARGET_BASENAME $SOURCE_BASENAME --transform "s,^$SOURCE_BASENAME,$TARGET_BASENAME_NO_EXT,"
