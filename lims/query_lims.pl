...
my %OUT_FIELDS_PER_TYPE = (
  'samples'     => [qw(submission sample_id sample_name reporting_id arrival_date label cohort analysis_type project_name lab_sop_versions lab_status)],
...
sub getUnreportedBiopsies{
    ...
    foreach my $sample_id ( keys %{$samples} ){
        ...
        next unless $sample->{'label'} =~ /^(CPCT|DRUP|WIDE|CORE|ACTN|SHRP|GAYA|OPTC|OMIC)$/;
