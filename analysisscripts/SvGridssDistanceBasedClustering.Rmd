...
cndf = query_all_copy_numer(dbgridss, "copyNumber190")
cngr = to_cn_gr(cndf)
svdf = query_somatic_structuralVariants(dbgridss, "structuralVariant190")
...
sdeldf = svdf %>% filter(id %in% sdelgr$id) %>%
  ...
  #filter(sampleId=="CPCT02020506TII") %>%
  dplyr::select(length, ihomlen, homlen, qualScore, inslen, ploidy, startAF, id, sampleId)
