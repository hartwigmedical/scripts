...
ploidyPenaltyFactor = 0.3  
standardDeviation = 0.05
minStandardDeviationPerPloidyPoint = 1
majorAlleleSubOnePenaltyMultiplier = 2
...
alleleDeviation <- function(purity, normFactor, ploidy) {
  ...
  return (2 * pnorm(ploidyDistanceFromInteger * standardDeviationsPerPloidy) - 1)
}
...
majorAlleleDeviation <- function(purity, normFactor, ploidy) {
  ...
  if (ploidy > 0 && ploidy < 1) {
    majorAlleleMultiplier = majorAlleleSubOnePenaltyMultiplier
  }
