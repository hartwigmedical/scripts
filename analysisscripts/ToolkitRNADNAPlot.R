...
plotColours = c('royal blue','skyblue3','lightblue','khaki4','khaki3','sienna1')
...
dnaRnaSummaryPlot = (ggplot(dnaRnaSummary , aes(x=KnownCategory, y=n, fill=MatchType))
                     ...
                     + theme(axis.text.x = element_text(angle=90, hjust=1,size=8),legend.text=element_text(size=8),axis.text.y = element_text(size=8),axis.title.x = element_text(size=8),legend.title=element_text(size=8))
                     + coord_flip())
