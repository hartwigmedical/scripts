...
ggplot(bind_rows(data.frame(set="Existing", called=matchdf$called, ploidy=matchdf$ploidy.old, transitive=matchdf$transitive),
								 data.frame(set="New", called=matchdf$called, ploidy=matchdf$ploidy.new, transitive=matchdf$transitive)) %>%
			 	filter(!is.na(ploidy) & transitive=="Neither")) +
	aes(x=ploidy, fill=called) +
	geom_histogram(bins=90*2) +
	facet_grid(transitive ~ set, scales="free") +
