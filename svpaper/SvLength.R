...
localPath = '~/hmf/analyses/SVAnalysis/archive/'
...
pCancerTypeDups = create_violin_plot_cancer_type(svEnriched %>% filter(Type %in% c("DUP"), ClusterCount == 1),feature='RepOriginStart') + ggtitle("Length Distribution: Simple DUP by CancerType (count per sample)")
pCancerTypeDels = create_violin_plot_cancer_type(svEnriched %>% filter(Type %in% c("DEL"), ClusterCount == 1),feature='RepOriginStart') + ggtitle("Simple DEL by CancerType (count per sample)")
plot_grid(pCancerTypeDups, pCancerTypeDels, ncol = 1)

create_top_n_violin_plot(svEnriched %>% filter(ResolvedType!='LINE',LEStart=='None'),10,feature='RepOriginStart',scaleLogY=F,minVariants = 10) + ggtitle("Rep Top CDK12 Dups")
head(svEnriched)
