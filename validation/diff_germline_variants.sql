SELECT
    MIN(pipeline) AS pipeline, chromosome, position, ref, alt, COUNT(*)
FROM
        (SELECT
            id, 'OnlyInTruth' AS pipeline, chromosome, position, ref, alt
        FROM
    ...
    UNION
        SELECT
            id, 'OnlyInNew' AS pipeline, chromosome, position, ref, alt
        FROM
