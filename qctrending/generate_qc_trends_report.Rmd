...
# Run QC Info
run_info <- as.data.frame(read.csv("/data/tmp/run_qc.csv", sep=",", header=T, na.strings=c("", "NA", "N/A")))
run_info$timestamp <- as.Date(run_info$timestamp, format = '%Y-%m-%dT%H:%M:%S')
run_info$type <- as.factor(run_info$type)
...
basic_table(coefficients_annotation(run_qc_info, "timestamp", "cov", "type"))
```
...
basic_table(coefficients_annotation(run_qc_info, "timestamp", "bqr", "type"))
```
