...
#codons
select sourceEvent, gene, chromosome, start, end, applicableMutationType,source, nctId, title, acronym, countriesAndCities, therapynames, genderCriterium, SUBSTRING_INDEX(indications,"(", 1) as cancerType, REPLACE(SUBSTRING_INDEX(indications,"(", -1),")", "") as doid  from actionableTrial
inner join trialMolecularCriterium on trialMolecularCriterium.actionableTrialid = actionableTrial.id
inner join actionableCodon on actionableCodon.molecularCriteriumId = trialMolecularCriterium.molecularCriteriumId;

#exons
select sourceEvent, gene, chromosome, start, end, applicableMutationType,source, nctId, title, acronym, countriesAndCities, therapynames, genderCriterium, SUBSTRING_INDEX(indications,"(", 1) as cancerType, REPLACE(SUBSTRING_INDEX(indications,"(", -1),")", "") as doid  from actionableTrial
inner join trialMolecularCriterium on trialMolecularCriterium.actionableTrialid = actionableTrial.id
inner join actionableExon on actionableExon.molecularCriteriumId = trialMolecularCriterium.molecularCriteriumId;

#fusions
select sourceEvent, geneUp, minExonUp, maxExonUp, geneDown, minExonDown, maxExonDown,source, nctId, title, acronym, countriesAndCities, therapynames, genderCriterium, SUBSTRING_INDEX(indications,"(", 1) as cancerType, REPLACE(SUBSTRING_INDEX(indications,"(", -1),")", "") as doid  from actionableTrial
inner join trialMolecularCriterium on trialMolecularCriterium.actionableTrialid = actionableTrial.id
inner join actionableFusion on actionableFusion.molecularCriteriumId = trialMolecularCriterium.molecularCriteriumId;

#genes
select sourceEvent, gene, event,source, nctId, title, acronym, countriesAndCities, therapynames, genderCriterium, SUBSTRING_INDEX(indications,"(", 1) as cancerType, REPLACE(SUBSTRING_INDEX(indications,"(", -1),")", "") as doid  from actionableTrial
inner join trialMolecularCriterium on trialMolecularCriterium.actionableTrialid = actionableTrial.id
inner join actionableGene on actionableGene.molecularCriteriumId = trialMolecularCriterium.molecularCriteriumId;

#hla
select sourceEvent, hlaAllele,source, nctId, title, acronym, countriesAndCities, therapynames, genderCriterium, SUBSTRING_INDEX(indications,"(", 1) as cancerType, REPLACE(SUBSTRING_INDEX(indications,"(", -1),")", "") as doid  from actionableTrial
inner join trialMolecularCriterium on trialMolecularCriterium.actionableTrialid = actionableTrial.id
inner join actionableHla on actionableHla.molecularCriteriumId = trialMolecularCriterium.molecularCriteriumId;