...
cupGender = cupPlotData  %>% filter(Category=='GENDER') %>% 
...
svPlot = ggplot(cupOtherData %>% filter(Category=='SV'),aes(x=RefCancerType,y=DataLabel)) +
...
traitsPlot = ggplot(cupOtherData %>% filter(Category=='SAMPLE_TRAIT'),aes(x=RefCancerType,y=DataLabel)) +
  geom_tile(aes(fill=PercColour),colour="grey",stat="identity",position="identity") +
  geom_text(aes(label=RefValueLabel),size=3) +
  scale_colour_manual(values=prevColours) +
  scale_fill_manual(values=prevColours,limits=names(prevColours)) +
...
} else
{
  ...
  plotHeights = c(10,195,45,85,105,85,featureHeight)
}
...
{
  grid.arrange(plot_grid(title,summaryPlot,genderPlot,svPlot,sigPlot,traitsPlot,
...
{
  plot_grid(title,summaryPlot,genderPlot,svPlot,sigPlot,traitsPlot,featurePlot,
