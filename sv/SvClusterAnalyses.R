...
svaDrivers = read.csv('~/logs/SVA_DRIVERS.csv')
nrow(svaDrivers)
View(svaDrivers)

# bring in cluster data
View(clusters)
svaDrivers = merge(svaDrivers,clusters,by=c('SampleId','ClusterId'),all.x=T)
View(svaDrivers)
View(svaDrivers %>% group_by(DriverType,SvMatched=SvId!='') %>% count())
View(svaDrivers %>% filter(DriverType=='BIALLELIC') %>% group_by(MatchInfo) %>% count())

View(svaDrivers %>% group_by(DriverType,FullyMatched) %>% count())
View(svaDrivers %>% filter(DriverType=='DEL') %>% group_by(FullyMatched) %>% count())
View(svaDrivers %>% filter(DriverType=='DEL'&FullyMatched=='false'))

matchedDrivers = svaDrivers %>% filter(FullyMatched=='true')
nrow(matchedDrivers)
matchedDrivers = merge(matchedDrivers,clusters,by=c('SampleId','ClusterId'),all.x=T)

View(matchedDrivers)
View(matchedDrivers %>% group_by(SampleId,GeneType,Gene) %>% count() %>% filter(n>1))

#######
## DEL drivers
delDrivers = matchedDrivers %>% filter(DriverType=='DEL')
delDrivers = delDrivers %>% mutate(MatchType=ifelse(MatchInfo=='MIN','MIN',ifelse(MatchInfo=='LOH',"LOH",'LOH_TC')))
View(delDrivers)

delDriverData = delDrivers %>% group_by(SampleId,GeneType,Gene,SvId,MatchType,ClusterId,
                                        ResolvedType,Synthetic,ClusterDesc,ClusterCount,FullyChained,ChainCount,HasReplicated,Foldbacks) %>% 
  summarise(Matches=n()) %>% mutate(SameSV=(Matches==2))

View(delDriverData)
View(delDriverData %>% group_by(MatchType,SameSV) %>% count())
View(delDriverData %>% filter(MatchType!='LOH_TC') %>% group_by(MatchType,SameSV,ResolvedType,Synthetic) %>% count())
View(delDriverData %>% group_by(MatchType,SameSV,ResolvedType) %>% count() %>% spread(ResolvedType,n))

# top TSGs
topTSGs = head(delDriverData %>% group_by(Gene) %>% summarise(TotalCount=n()) %>% arrange(-TotalCount),10)
View(topTSGs)

delDriverSummary = delDriverData %>% filter(Gene %in% topTSGs$Gene) %>% 
  mutate(DelType=ifelse(MatchType=='LOH_TC','ARM LOSS','SV'),
         ClusterType=ifelse(DelType=='ARM LOSS', '',ifelse(ResolvedType=='DEL'&SameSV,'SIMPLE DEL','COMPLEX'))) %>%
 group_by(Gene,DelType,ClusterType) %>% summarise(Count=n()) 

delDriverSummary = merge(delDriverSummary,topTSGs,by='Gene',all.x=T)
View(delDriverSummary %>% arrange(-TotalCount,-Count))

# most common TSGs and SV & cluster types
View(delDriverData %>% group_by(Gene,MatchType,SameSV,ResolvedType) %>% count())

# some examples
View(delDriverData %>% filter(Gene=='CDKN2A'&MatchType=='MIN'&!SameSV))
View(delDriverData %>% filter(Gene=='CDKN2A'&MatchType=='MIN'&!SameSV&Foldbacks==0&ChainCount==1&HasReplicated=='false'&FullyChained=='true'))

# check for different clusters in either the MIN or LOH pairing - all are in the same cluster
View(delDriverData %>% filter(!SameSV&MatchType!='LOH_TC') %>% group_by(SampleId,Gene,MatchType) %>% 
       summarise(SameCluster=sum(first(ClusterId)==last(ClusterId))) %>% group_by(MatchType,SameCluster) %>% count())


#####
## AMPs
ampMatches = matchedDrivers %>% filter(DriverType=='AMP') %>% group_by(SampleId,Gene) %>% 
  summarise(ClusterId=first(ClusterId),DupCount=sum(MatchInfo=='SV')/2,TICount=sum(MatchInfo=='TI')/2)

View(ampMatches)

topONCOs = head(ampMatches %>% group_by(Gene) %>% summarise(TotalCount=n()) %>% arrange(-TotalCount),20)
View(topONCOs)

# top 20 oncogens and the type of cluster which caused them
ampDriverSummary = ampMatches %>% filter(Gene %in% topONCOs$Gene) %>% 
  merge(clusters %>% select(SampleId,ClusterId,ResolvedType,Synthetic,ClusterDesc,ClusterCount,FullyChained,ChainCount,Replication,Foldbacks),
        by=c('SampleId','ClusterId'),all.x=T) %>%
  mutate(ClusterType=ifelse(ResolvedType=='DUP'&TICount==0,'DUP',ifelse(Foldbacks>0,'BFB','COMPLEX'))) %>%
  group_by(Gene,ClusterType) %>% count()

ampDriverSummary2 = ampDriverSummary %>% filter(!is.na(ClusterType)) %>% spread(ClusterType,n)
ampDriverSummary2[is.na(ampDriverSummary2)] = 0
ampDriverSummary2 = merge(ampDriverSummary2,topONCOs,by='Gene',all.x=T)
View(ampDriverSummary2)

View(svData %>% filter(SampleId=='CPCT02070433T'&(ClusterId %in% c(162,167,172))))

View(sampleCancerTypes)

View(matchedDrivers %>% filter(DriverType=='AMP'&MatchInfo=='TI') %>% group_by(SampleId,GeneType,Gene) %>% count())
View(matchedDrivers %>% filter(DriverType=='AMP'&MatchInfo=='SV') %>% group_by(SampleId,GeneType,Gene) %>% count())

View(ampMatches)
View(matchedDrivers %>% filter(SampleId=='CPCT02010762T'&Gene=='MDM2')) # 7 overlapping DUPs
View(matchedDrivers %>% filter(SampleId=='CPCT02020512T'&Gene=='MDM2'))
View(matchedDrivers %>% filter(SampleId=='DRUP01010034T'&Gene=='ERBB2'))
View(clusters %>% filter(SampleId=='CPCT02070192T'&ClusterId==57))

View(matchedDrivers %>% filter(DriverType=='AMP'&MatchInfo=='TI'&ChainCount==1&FullyChained=='true') %>% 
       group_by(SampleId,ClusterId,DriverType,Gene,ResolvedType,ClusterDesc,MaxCopyNumber) %>% count())


View(ampMatches %>% group_by(Gene,MatchInfo) %>% count())
