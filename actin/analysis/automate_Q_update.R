...
wd <- "/Users/charlottebruel/Downloads/"
...
pdf(file= paste(wd,"LR_CUP.pdf",sep=""), width = 10, height = 7)
par(mar=c(5,5,5,5),mfrow=c(1,1))
barplot(as.matrix(LR_CUP), xlim = c(0,max(LR_CUP[1,])+max(LR_CUP[2,])+5), col=c("blue","red"), las=1, legend = c("LR", "CUP"), horiz = TRUE, args.legend = list(x ='topright', inset = c(-0.05,0.05)), main = paste("Overall, ", total_nr_biopsies, " biopsies (", total_nr_biopsies_LR, " LR, ", total_nr_biopsies_CUP, " CUP) for ", total_nr_patients_value, " patients have been evaluated in ACTIN ", sep = ""))
grid(nx=NULL,ny=NA,lty=1,col="gray",lwd=1)
barplot(as.matrix(LR_CUP), xlim = c(0,max(LR_CUP[1,])+max(LR_CUP[2,])+5), col=c("blue","red"), las=1, legend = c("LR", "CUP"), horiz = TRUE, args.legend = list(x ='topright', inset = c(-0.05,0.05)), main = paste("Overall, ", total_nr_biopsies, " biopsies (", total_nr_biopsies_LR, " LR, ", total_nr_biopsies_CUP, " CUP) for ", total_nr_patients_value, " patients have been evaluated in ACTIN ", sep = ""), add=TRUE)
dev.off()
...
pdf(file= paste(wd,"sufficient_tumor_cells.pdf",sep=""), width = 10, height = 7)
par(mar=c(5,5,5,5)+.1)
barplot(as.matrix(suff_cells), xlim = c(0,100), col=c("blue","red"), las=1, horiz = TRUE, main = paste(round(sufficient_nr), "% of biopsies contain sufficient tumor cells for WGTS analysis", sep = ""), xaxt = "n")
grid(nx=NULL,ny=NA,lty=1,col="gray",lwd=1)
barplot(as.matrix(suff_cells), xlim = c(0,100), col=c("blue","red"), las=1, horiz = TRUE, main = paste(round(sufficient_nr), "% of biopsies contain sufficient tumor cells for WGTS analysis", sep = ""), add=TRUE, xaxt = "n")
...
pdf(file= paste(wd,"sufficient_tumor_cells_LR.pdf",sep=""), width = 10, height = 7)
par(mar=c(5,5,5,5)+.1)
...
pdf(file= paste(wd,"sufficient_tumor_cells_CUP.pdf",sep=""), width = 10, height = 7)
par(mar=c(5,5,5,5)+.1)
...
pdf(file= paste(wd,"TAT_calender_days.pdf",sep=""), width = 10, height = 7)
barplot(as.matrix(rev(tat_calender)), ylab = "TAT calender days", ylim = c(0,max(tat_calender)+1), col = c("blue"), main = paste("Average TAT (turnaround-time) in calendar days for WGS is ", average_tat, " days", sep = ""))
grid(nx=NA,ny=NULL,lty=1,col="gray",lwd=1)
barplot(as.matrix(rev(tat_calender)), ylab = "TAT calender days", ylim = c(0,max(tat_calender)+1), col = c("blue"), main = paste("Average TAT (turnaround-time) in calendar days for WGS is ", average_tat, " days", sep = ""), add=TRUE)
dev.off()
...
pdf(file= paste(wd,"WGS_allowed_therapy.pdf",sep=""), width = 10, height = 7)
...
pdf(file= paste(wd,"treated_based_on_WGS.pdf",sep=""), width = 10, height = 7)
...
pdf(file= paste(wd,"WGS_impact_category.pdf",sep=""), width = 20, height = 7)
par(mar=c(5,25,5,5))
...
pdf(file= paste(wd,"WGS_CUP.pdf",sep=""), width = 10, height = 7)
par(mar=c(5,7,5,5))
pie(slices,labels = lbls, col=c("orange","blue","red"),main = paste("For ", nr_confirmation_or_diagnosis, "% CUP patients WGS led to confirmation (", nr_confirmation, "%) or actual diagnosis (", nr_actual_diagnosis, "%)", sep = ""))
dev.off()
...
pdf(file=paste(wd,"WTS_CUP.pdf",sep=""), width = 10, height = 3)
...
pdf(file=paste(wd,"WTS_LR.pdf",sep=""), width = 10, height = 3)
